...
class ShipRegistryModuleTest {
    ...
    void installModule_fits_module_into_free_slot() {
        ...
        Ship updated = registry.installModule(ship.identity(), ModuleType.CARGO, ModuleSlot.BOW);

        assertEquals(Map.of(ModuleSlot.BOW, ModuleType.CARGO), updated.modules());
        assertEquals(Map.of(ModuleSlot.BOW, ModuleType.CARGO),
    ...
    void installModule_refuses_occupied_slot() {
        ...
        assertEquals(ModuleType.CARGO,
                registry.find(ship.identity()).orElseThrow().modules().get(ModuleSlot.BOW));
    }
    ...
    void removeModule_clears_slot() {
        ...
        Ship updated = registry.removeModule(ship.identity(), ModuleSlot.PORT);
    ...
    void removeModule_refuses_empty_slot() {
        ...
        IllegalStateException ex = assertThrows(IllegalStateException.class,
                () -> registry.removeModule(ship.identity(), ModuleSlot.BOW));
    ...
    void moveModule_relocates_module_to_free_slot() {
        ...
        Ship updated = registry.moveModule(ship.identity(), ModuleSlot.BOW, ModuleSlot.STERN);

        assertFalse(updated.modules().containsKey(ModuleSlot.BOW));
        assertEquals(ModuleType.CANNON, updated.modules().get(ModuleSlot.STERN));
    ...
    void moveModule_refuses_occupied_target() {
        ...
        assertThrows(IllegalStateException.class,
                () -> registry.moveModule(ship.identity(), ModuleSlot.BOW, ModuleSlot.STERN));
    }
    ...
    void damage_preserves_installed_modules() {
        ...
        assertEquals(Map.of(ModuleSlot.BOW, ModuleType.CARGO), after.modules());
    }
}
